...
class flooring_top_bar_walker extends Walker_Nav_Menu
{
    ...
    function start_el(&$output, $item, $depth = 0, $args = Array(), $id = 0)
    {
        ...
        if (is_object($args)) {
            ...
            $flooring_icon = $item->mega_menu_icon;
            if ($item->mega_menu == 1) {
                $flooring_class = 'wd_mega-menu';
            }
			...
			<a' . $attributes . '>' : '';
            if (($flooring_icon != '') and ($flooring_icon != '---- None ----')) {

                $item_output .= '<i class="' . $flooring_icon . ' fa"></i> ';
            }
...
if (!function_exists('flooring_add_mega_menu_support')):

    function flooring_add_mega_menu_support()
    {


        add_filter('wp_setup_nav_menu_item', 'flooring_add_custom_nav_fields');

        add_action('wp_update_nav_menu_item', 'flooring_update_custom_nav_fields', 10, 3);

        add_filter('wp_edit_nav_menu_walker', 'flooring_edit_menu_walker', 10, 2);
...
add_action('init', 'flooring_add_mega_menu_support');


/* Add custom fields to menu */

if (!function_exists('flooring_add_custom_nav_fields')):

    function flooring_add_custom_nav_fields($menu_item)
    {


        $menu_item->mega_menu_icon = get_post_meta($menu_item->ID, '_flooring_mega_menu_icon', true) ? get_post_meta($menu_item->ID, '_flooring_mega_menu_icon', true) : '';

        $menu_item->mega_menu_bg_image = get_post_meta($menu_item->ID, '_flooring_mega_menu_bg_image', true) ? get_post_meta($menu_item->ID, '_flooring_mega_menu_bg_image', true) : '';

        $menu_item->mega_menu = get_post_meta($menu_item->ID, '_flooring_mega_menu', true) ? 1 : 0;
...
if (!function_exists('flooring_update_custom_nav_fields')):

    function flooring_update_custom_nav_fields($menu_id, $menu_item_db_id, $args)
    {


        $value = isset($_REQUEST['menu-item-mega-menu-icon'][$menu_item_db_id]) ? $_REQUEST['menu-item-mega-menu-icon'][$menu_item_db_id] : '';
        update_post_meta($menu_item_db_id, '_flooring_mega_menu_icon', $value);


        $value = isset($_REQUEST['menu-item-mega-menu-bg-image'][$menu_item_db_id]) ? $_REQUEST['menu-item-mega-menu-bg-image'][$menu_item_db_id] : '';
        update_post_meta($menu_item_db_id, '_flooring_mega_menu_bg_image', $value);


        $value = isset($_REQUEST['menu-item-mega-menu'][$menu_item_db_id]) ? 1 : 0;
        update_post_meta($menu_item_db_id, '_flooring_mega_menu', $value);
...
if (!function_exists('flooring_edit_menu_walker')):

    function flooring_edit_menu_walker($walker, $menu_id)
    {


        class flooring_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit
        {
            ...
            public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
            {
                ...
                $mega_menu_html .= '</select>
		                ...
		                ' . __('display icone', 'flooring') . '</label>
                ...
                $mega_menu_html .= '<p class="field-custom description description-wide">
			                ...
			                ' . __('Mega Menu (Background image)', 'flooring') . '
        ...
        return 'flooring_Walker_Nav_Menu_Edit';
